...
const App = () => {
    ...
    if(removeSpaces(calc.num).length < 16){
      setCalc({
        ...
        num:
          ...
          : removeSpaces(calc.num) % 1 === 0 ? toLocalString(Number(removeSpaces(calc.num + value)))
  ...
  const equalsClickHandler = () => {
    ...
    setCalc({
      ...calc,
      res:
        calc.num === "0" && calc.num ==="/" ? "Cant divide with 0"
        : toLocalString(
            math(
  ...
  const invertClickHandler = () => {
    setCalc({
      ...
      res: calc.num ? toLocalString(removeSpaces(calc.res) * -1) : 0,
      sign: "",
    });
  };
